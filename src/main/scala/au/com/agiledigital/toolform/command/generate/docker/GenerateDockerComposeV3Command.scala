...
object GenerateDockerComposeV3Command extends YamlWriter {
  ...
  def runGenerateDockerComposeV3(sourceFilePath: String, outFile: File, project: Project): Either[NonEmptyList[ToolFormError], String] = {
    ...
    // check that image names are not 'None', returning a NonEmptyList[ToolFormError] for those that are.
    val imageNameErrors = for (resource <- project.sortedResources.values.toList if (resource.image == None))
      yield (resource.id)
    val errors = imageNameErrors.collect { case (a: String) => a }.mkString(", ")

    if (!errors.isEmpty)
      Left(NonEmptyList.of(ToolFormError(s"Image name does not exist for: $errors")))
    else {
