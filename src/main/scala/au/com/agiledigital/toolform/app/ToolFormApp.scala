    ...
    if (!projectDefinitionFile.exists()) {
      ...
      Try(ConfigFactory.parseFile(projectDefinitionFile)) match {
        case Success(simpleConfig) =>
          ...
          implicit val refConvert: ConfigConvert[Reference] =
            viaStringOpt(s => Some(Reference(s)), _.toString)
          implicit val resourceFieldMapping = ProductHint[Resource](new ConfigFieldMapping {
            def apply(fieldName: String) = if (fieldName == "resourceType") "resource_type" else fieldName
          })
          implicit val edgeFieldMapping = ProductHint[Edge](new ConfigFieldMapping {
            def apply(fieldName: String) = fieldName
          })
          implicit val subEdgeFieldMapping = ProductHint[SubEdge](new ConfigFieldMapping {
            def apply(fieldName: String) = if (fieldName == "edgeType") "type" else fieldName
          })
          implicit val locationFieldMapping = ProductHint[Location](new ConfigFieldMapping {
            def apply(fieldName: String) = fieldName
          })
...
/**
  ...
  */
case class ToolFormConfiguration(in: File = new File("."), inspect: Boolean = false)
